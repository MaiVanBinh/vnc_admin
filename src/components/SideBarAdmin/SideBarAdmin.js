import React from "react";
import "./SideBarAdmin.css";
import { useHistory } from "react-router";
import { connect } from "react-redux";
import {
  IconWood,
  IconFootPrint,
  IconFeedbacks,
  IconQuestion,
  IconEvent,
  IconContributions,
} from "../../store/utilities/SVG";

const mapStateToProps = (state) => {
  return {
    auth: state.auth,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {};
};

const SideBarAdmin = (props) => {
  const { auth, page, setPage } = props;
  const history = useHistory();

  const changePage = (pageRoute) => {
    history.push(pageRoute);
    if (pageRoute === "/dashboard") setPage(0);
    else if (pageRoute === "/danh-muc") setPage(1);
    else if (pageRoute === "/bai-viet") setPage(2);
    else if (pageRoute === "/quan-li-anh") setPage(3);
    else if (pageRoute === "/phan-loai-dong-vat") setPage(4);
    else if (pageRoute === "/users" || pageRoute === "/tai-khoan") setPage(5);
    else if (pageRoute === "/quan-li-sinh-vat") setPage(6);
    else if (pageRoute === "/dau-chan") setPage(8);
    else if (pageRoute === "/mau-go") setPage(7);
    else if (pageRoute === "/feedbacks") setPage(9);
    else if (pageRoute === "/questions") setPage(10);
    else if (pageRoute === "/events") setPage(11);
    else if (pageRoute === "/contributions") setPage(12);
    else setPage(0);
  };

  return (
    <div className="main-menu-container">
      <div
        className={page === 0 ? "menu-item active" : "menu-item"}
        onClick={() => changePage("/dashboard")}
      >
        <span>Bảng điều khiển</span>
      </div>
      <div
        className={page === 1 ? "menu-item active" : "menu-item"}
        onClick={() => changePage("/danh-muc")}
      >
        <span className="icon">
          <svg
            height="20px"
            viewBox="0 -24 415.329 415"
            width="20px"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              fill={"#555"}
              d="M368.328 130.164h-26.39V88.52c-.036-27.301-22.157-49.426-49.461-49.457H223.09a32.974 32.974 0 01-21.664-8.106l-22.11-19.246A47.014 47.014 0 00148.457.164h-99C22.157.195.031 22.32 0 49.621v260.754c.035 30.258 23.715 55.215 53.93 56.84v.078h280.203c22.375-.043 41.637-15.82 46.078-37.754l34.168-143 .059-.27a46.995 46.995 0 00-9.797-38.945 47.005 47.005 0 00-36.313-17.16zM14.004 310.375V49.621c.023-19.574 15.883-35.433 35.457-35.457h99c7.965 0 15.66 2.879 21.668 8.11l22.11 19.242a46.994 46.994 0 0030.85 11.547h69.388c19.574.023 35.437 15.886 35.46 35.46v41.641h-6.652c-3.394-18.14-19.219-31.3-37.676-31.328h-2.312c-3.402-18.516-19.531-31.973-38.36-32h-161c-25.945.031-46.972 21.055-47 47v233.379a42.926 42.926 0 01-20.933-36.84zm269.605-197.54a24.365 24.365 0 0123.301 17.33H188.016c-21.746-.067-40.672 14.847-45.7 36.003L98.758 319.98l-.078.293A42.87 42.87 0 0188.938 339V145.836c.019-18.219 14.785-32.98 33-33zm-234.671 239.7v-238.7c.019-18.218 14.785-32.98 33-33h161a25.036 25.036 0 0124 18h-145c-25.946.028-46.97 21.056-47 47v203.52a43.013 43.013 0 01-26 3.18zm351.792-169.11l-34.164 142.99-.058.273c-3.082 15.445-16.63 26.574-32.375 26.605H94.39a56.806 56.806 0 0017.879-29.64l43.558-153.81.078-.292c3.489-14.903 16.801-25.43 32.11-25.387h180.312c9.852 0 19.184 4.399 25.453 11.996s8.817 17.598 6.95 27.27zm0 0"
            />
          </svg>
        </span>
        <span>Danh mục</span>
      </div>
      <div
        className={page === 2 ? "menu-item active" : "menu-item"}
        onClick={() => changePage("/bai-viet")}
      >
        <span className="icon">
          <svg
            height="25px"
            viewBox="-61 0 484 484.375"
            width="25px"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path d="M6.383 484.375h349.61c3.312 0 6.195-2.313 6.195-5.625V66.555a6.327 6.327 0 00-6.196-6.18h-44.805V48.344c0-3.313-3.07-5.969-6.382-5.969h-30.618V6a6.001 6.001 0 00-12 0v36.375H232.41a6.168 6.168 0 00-6.222 5.969v12.031h-90V48.344a6.168 6.168 0 00-6.223-5.969h-29.778V6a6.001 6.001 0 00-12 0v36.375H57.57c-3.312 0-6.382 2.656-6.382 5.969v12.031H6.383a6.327 6.327 0 00-6.195 6.18V478.75c0 3.313 2.882 5.625 6.195 5.625zm231.804-430h61v22h-61zm-175 0h61v22h-61zm-51 18h39v10.117c0 3.313 3.07 5.883 6.383 5.883h72.395a6.1 6.1 0 006.222-5.883V72.375h90v10.117a6.1 6.1 0 006.223 5.883h72.395c3.312 0 6.382-2.57 6.382-5.883V72.375h39v400h-338zm0 0" />
            <path d="M303.21 381.375H59.165c-3.312 0-6 2.688-6 6s2.688 6 6 6h244.047c3.312 0 6-2.688 6-6s-2.688-6-6-6zm0 0M303.21 425.375H59.165c-3.312 0-6 2.688-6 6s2.688 6 6 6h244.047c3.312 0 6-2.688 6-6s-2.688-6-6-6zm0 0M308.188 134.375c0-3.313-2.688-6-6-6h-242c-3.313 0-6 2.688-6 6v205c0 3.313 2.687 6 6 6h242c3.312 0 6-2.688 6-6zm-242 6h230v193h-230zm0 0" />
            <path d="M213.625 205.828l-10.438-1.11v-21.573a5.843 5.843 0 00-2.351-4.903 5.777 5.777 0 00-5.367-.547l-66.781 23.68h-33.93c-6.38 0-11.57 4.75-11.57 11.129v33.766c0 4.5 3 8.406 6 10.316v33.02c0 3.316 3.046 5.769 6.359 5.769h19.351a5.898 5.898 0 005.754-4.188l9.7-32.82 65.203 22.899c.636.23 1.308.347 1.984.351a5.446 5.446 0 003.293-1.097 5.836 5.836 0 002.356-4.891V252.57l10.437-1.113c8.754-.918 15.441-8.238 15.563-17.043v-11.539c-.118-8.805-6.805-16.129-15.563-17.047zm-118.438 7.547h29v32h-29zm15.227 70h-9.226v-26h16.828zm80.773-16.207l-55-19.582v-36.402l55-19.582zm26-32.754a5.396 5.396 0 01-4.87 5.113l-9.13.97v-23.708l9.13.969a5.403 5.403 0 014.87 5.117zm0 0M274.188 223.375h-33c-3.313 0-6 2.688-6 6s2.687 6 6 6h33c3.312 0 6-2.688 6-6s-2.688-6-6-6zm0 0M238.688 215.422c.503 0 1.007-.063 1.492-.192l23.363-5.988a5.998 5.998 0 004.32-7.3 6.003 6.003 0 00-7.3-4.325l-23.36 5.992a5.999 5.999 0 001.484 11.813zm0 0M263.54 250.328l-23.36-5.988a6.002 6.002 0 00-2.98 11.625l23.359 5.988a5.994 5.994 0 007.3-4.32 6 6 0 00-4.32-7.305zm0 0" />
          </svg>
        </span>
        <span>Bài viết</span>
      </div>
      <div
        className={page === 3 ? "menu-item active" : "menu-item"}
        onClick={() => changePage("/quan-li-anh")}
      >
        <span className="icon">
          <svg
            height="23px"
            viewBox="0 0 24 24"
            width="23px"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path d="M17.453 24c-.168 0-.34-.021-.51-.066L1.48 19.793a2.018 2.018 0 01-1.414-2.45l1.951-7.272a.5.5 0 01.966.258l-1.95 7.27c-.139.53.179 1.082.71 1.229L17.2 22.967a.995.995 0 001.217-.704l.781-2.894a.499.499 0 11.966.26l-.78 2.89A1.997 1.997 0 0117.453 24z" />
            <path d="M22 18H6c-1.103 0-2-.897-2-2V4c0-1.103.897-2 2-2h16c1.103 0 2 .897 2 2v12c0 1.103-.897 2-2 2zM6 3c-.551 0-1 .449-1 1v12c0 .551.449 1 1 1h16c.551 0 1-.449 1-1V4c0-.551-.449-1-1-1z" />
            <path d="M9 9c-1.103 0-2-.897-2-2s.897-2 2-2 2 .897 2 2-.897 2-2 2zm0-3c-.551 0-1 .449-1 1s.449 1 1 1 1-.449 1-1-.449-1-1-1zM4.57 16.93a.5.5 0 01-.354-.853l4.723-4.723c.566-.566 1.555-.566 2.121 0l1.406 1.406 3.892-4.67a1.502 1.502 0 011.142-.54h.011a1.5 1.5 0 011.139.523l5.23 6.102a.5.5 0 11-.759.651l-5.23-6.102a.495.495 0 00-.38-.174.53.53 0 00-.384.18l-4.243 5.091a.498.498 0 01-.362.179.481.481 0 01-.376-.146l-1.793-1.793a.514.514 0 00-.707 0l-4.723 4.723a.498.498 0 01-.353.146z" />
          </svg>
        </span>
        <span>Quản lí ảnh</span>
      </div>
      <div
        className={page === 4 ? "menu-item active" : "menu-item"}
        onClick={() => changePage("/phan-loai-dong-vat")}
      >
        <span className="icon">
          <svg
            aria-hidden="true"
            data-prefix="fas"
            data-icon="network-wired"
            className="svg-inline--fa fa-network-wired fa-w-20"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 640 512"
            height="23px"
            width="23px"
          >
            <path
              fill="currentColor"
              d="M640 264v-16c0-8.84-7.16-16-16-16H344v-40h72c17.67 0 32-14.33 32-32V32c0-17.67-14.33-32-32-32H224c-17.67 0-32 14.33-32 32v128c0 17.67 14.33 32 32 32h72v40H16c-8.84 0-16 7.16-16 16v16c0 8.84 7.16 16 16 16h104v40H64c-17.67 0-32 14.33-32 32v128c0 17.67 14.33 32 32 32h160c17.67 0 32-14.33 32-32V352c0-17.67-14.33-32-32-32h-56v-40h304v40h-56c-17.67 0-32 14.33-32 32v128c0 17.67 14.33 32 32 32h160c17.67 0 32-14.33 32-32V352c0-17.67-14.33-32-32-32h-56v-40h104c8.84 0 16-7.16 16-16zM256 128V64h128v64H256zm-64 320H96v-64h96v64zm352 0h-96v-64h96v64z"
            />
          </svg>
        </span>
        <span>Phân loại động vật</span>
      </div>

      {auth.user && auth.user.role === "1" ? ( // super admin
        <div
          className={page === 5 ? "menu-item active" : "menu-item"}
          onClick={() => changePage("/users")}
        >
          <span className="icon">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 480 480"
              width="23px"
              height="23px"
            >
              <path d="M240 0C107.664 0 0 107.664 0 240c0 57.96 20.656 111.184 54.992 152.704.088.12.096.272.192.384 24.792 29.896 55.928 52.816 90.624 67.624.4.168.792.352 1.192.52 2.808 1.184 5.648 2.28 8.496 3.352 1.12.424 2.24.856 3.376 1.264 2.456.88 4.928 1.712 7.416 2.512 1.592.512 3.184 1.016 4.792 1.496 2.2.656 4.408 1.288 6.632 1.888 1.952.528 3.92 1.016 5.888 1.488 1.992.48 3.992.96 6 1.384 2.24.48 4.504.904 6.776 1.32 1.824.336 3.64.688 5.48.984 2.52.408 5.056.728 7.6 1.056 1.64.208 3.272.448 4.92.624 2.88.304 5.784.52 8.696.72 1.352.096 2.696.24 4.056.312 4.248.24 8.544.368 12.872.368s8.624-.128 12.888-.352c1.36-.072 2.704-.216 4.056-.312 2.912-.208 5.816-.416 8.696-.72 1.648-.176 3.28-.416 4.92-.624 2.544-.328 5.08-.648 7.6-1.056 1.832-.296 3.656-.648 5.48-.984 2.264-.416 4.528-.84 6.776-1.32 2.008-.432 4-.904 6-1.384 1.968-.48 3.936-.968 5.888-1.488a232.661 232.661 0 0011.424-3.384c2.488-.8 4.96-1.632 7.416-2.512 1.128-.408 2.248-.84 3.376-1.264a243.168 243.168 0 008.496-3.352c.4-.168.792-.352 1.192-.52a239.997 239.997 0 0090.624-67.624c.096-.112.104-.272.192-.384C459.344 351.184 480 297.96 480 240 480 107.664 372.336 0 240 0zm97.256 441.76c-.12.056-.232.12-.352.176-2.856 1.376-5.76 2.672-8.688 3.936-.664.28-1.32.568-1.984.848a217.023 217.023 0 01-7.76 3.064 159.85 159.85 0 01-3.272 1.192 230.572 230.572 0 01-6.976 2.368c-1.456.464-2.92.904-4.384 1.336a202.08 202.08 0 01-6.28 1.784c-1.776.472-3.568.904-5.36 1.328-1.88.448-3.752.904-5.648 1.304-2.072.44-4.16.816-6.24 1.192-1.688.312-3.368.64-5.072.912-2.344.368-4.712.664-7.072.96-1.496.192-2.984.416-4.496.576-2.696.288-5.416.472-8.128.664-1.208.08-2.408.216-3.632.28-3.96.208-7.928.32-11.912.32s-7.952-.112-11.904-.32c-1.216-.064-2.416-.192-3.632-.28-2.72-.184-5.432-.376-8.128-.664-1.512-.16-3-.384-4.496-.576-2.36-.296-4.728-.592-7.072-.96-1.704-.272-3.384-.6-5.072-.912-2.088-.376-4.176-.76-6.24-1.192-1.896-.4-3.776-.856-5.648-1.304-1.792-.432-3.584-.856-5.36-1.328-2.104-.56-4.2-1.168-6.28-1.784a212.543 212.543 0 01-4.384-1.336 223.256 223.256 0 01-6.976-2.368 217.023 217.023 0 01-11.032-4.256c-.664-.272-1.312-.56-1.976-.84a223.812 223.812 0 01-8.696-3.936 7.216 7.216 0 01-.352-.176c-27.912-13.504-52.568-32.672-72.576-55.952 15.464-56.944 59.24-102.848 115.56-121.112 1.112.68 2.272 1.288 3.416 1.928.672.376 1.336.776 2.016 1.136a99.592 99.592 0 007.272 3.512c1.896.832 3.856 1.536 5.808 2.256.384.136.768.288 1.152.424 10.848 3.84 22.456 6.04 34.6 6.04s23.752-2.2 34.592-6.04c.384-.136.768-.288 1.152-.424 1.952-.72 3.912-1.424 5.808-2.256a99.592 99.592 0 007.272-3.512c.68-.368 1.344-.76 2.016-1.136 1.144-.64 2.312-1.248 3.432-1.936 56.32 18.272 100.088 64.176 115.56 121.112-20.008 23.272-44.664 42.44-72.576 55.952zM152 176c0-48.52 39.48-88 88-88s88 39.48 88 88c0 30.864-16.008 58.024-40.128 73.736a84.742 84.742 0 01-9.8 5.48c-.4.192-.792.392-1.192.576-23.168 10.536-50.592 10.536-73.76 0-.4-.184-.8-.384-1.192-.576a84.083 84.083 0 01-9.8-5.48C168.008 234.024 152 206.864 152 176zm269.832 194.584c-18.136-53.552-59.512-96.832-112.376-117.392C330.6 234.144 344 206.64 344 176c0-57.344-46.656-104-104-104s-104 46.656-104 104c0 30.64 13.4 58.144 34.552 77.192-52.864 20.568-94.24 63.84-112.376 117.392C31.672 333.792 16 288.704 16 240 16 116.488 116.488 16 240 16s224 100.488 224 224c0 48.704-15.672 93.792-42.168 130.584z" />
            </svg>
          </span>
          <span>Người dùng</span>
        </div>
      ) : null}

      {auth.user && auth.user.role === "0" ? ( // admin
        <div
          className={page === 5 ? "menu-item active" : "menu-item"}
          onClick={() => changePage("/tai-khoan")}
        >
          <span className="icon">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 480 480"
              width="23px"
              height="23px"
            >
              <path d="M240 0C107.664 0 0 107.664 0 240c0 57.96 20.656 111.184 54.992 152.704.088.12.096.272.192.384 24.792 29.896 55.928 52.816 90.624 67.624.4.168.792.352 1.192.52 2.808 1.184 5.648 2.28 8.496 3.352 1.12.424 2.24.856 3.376 1.264 2.456.88 4.928 1.712 7.416 2.512 1.592.512 3.184 1.016 4.792 1.496 2.2.656 4.408 1.288 6.632 1.888 1.952.528 3.92 1.016 5.888 1.488 1.992.48 3.992.96 6 1.384 2.24.48 4.504.904 6.776 1.32 1.824.336 3.64.688 5.48.984 2.52.408 5.056.728 7.6 1.056 1.64.208 3.272.448 4.92.624 2.88.304 5.784.52 8.696.72 1.352.096 2.696.24 4.056.312 4.248.24 8.544.368 12.872.368s8.624-.128 12.888-.352c1.36-.072 2.704-.216 4.056-.312 2.912-.208 5.816-.416 8.696-.72 1.648-.176 3.28-.416 4.92-.624 2.544-.328 5.08-.648 7.6-1.056 1.832-.296 3.656-.648 5.48-.984 2.264-.416 4.528-.84 6.776-1.32 2.008-.432 4-.904 6-1.384 1.968-.48 3.936-.968 5.888-1.488a232.661 232.661 0 0011.424-3.384c2.488-.8 4.96-1.632 7.416-2.512 1.128-.408 2.248-.84 3.376-1.264a243.168 243.168 0 008.496-3.352c.4-.168.792-.352 1.192-.52a239.997 239.997 0 0090.624-67.624c.096-.112.104-.272.192-.384C459.344 351.184 480 297.96 480 240 480 107.664 372.336 0 240 0zm97.256 441.76c-.12.056-.232.12-.352.176-2.856 1.376-5.76 2.672-8.688 3.936-.664.28-1.32.568-1.984.848a217.023 217.023 0 01-7.76 3.064 159.85 159.85 0 01-3.272 1.192 230.572 230.572 0 01-6.976 2.368c-1.456.464-2.92.904-4.384 1.336a202.08 202.08 0 01-6.28 1.784c-1.776.472-3.568.904-5.36 1.328-1.88.448-3.752.904-5.648 1.304-2.072.44-4.16.816-6.24 1.192-1.688.312-3.368.64-5.072.912-2.344.368-4.712.664-7.072.96-1.496.192-2.984.416-4.496.576-2.696.288-5.416.472-8.128.664-1.208.08-2.408.216-3.632.28-3.96.208-7.928.32-11.912.32s-7.952-.112-11.904-.32c-1.216-.064-2.416-.192-3.632-.28-2.72-.184-5.432-.376-8.128-.664-1.512-.16-3-.384-4.496-.576-2.36-.296-4.728-.592-7.072-.96-1.704-.272-3.384-.6-5.072-.912-2.088-.376-4.176-.76-6.24-1.192-1.896-.4-3.776-.856-5.648-1.304-1.792-.432-3.584-.856-5.36-1.328-2.104-.56-4.2-1.168-6.28-1.784a212.543 212.543 0 01-4.384-1.336 223.256 223.256 0 01-6.976-2.368 217.023 217.023 0 01-11.032-4.256c-.664-.272-1.312-.56-1.976-.84a223.812 223.812 0 01-8.696-3.936 7.216 7.216 0 01-.352-.176c-27.912-13.504-52.568-32.672-72.576-55.952 15.464-56.944 59.24-102.848 115.56-121.112 1.112.68 2.272 1.288 3.416 1.928.672.376 1.336.776 2.016 1.136a99.592 99.592 0 007.272 3.512c1.896.832 3.856 1.536 5.808 2.256.384.136.768.288 1.152.424 10.848 3.84 22.456 6.04 34.6 6.04s23.752-2.2 34.592-6.04c.384-.136.768-.288 1.152-.424 1.952-.72 3.912-1.424 5.808-2.256a99.592 99.592 0 007.272-3.512c.68-.368 1.344-.76 2.016-1.136 1.144-.64 2.312-1.248 3.432-1.936 56.32 18.272 100.088 64.176 115.56 121.112-20.008 23.272-44.664 42.44-72.576 55.952zM152 176c0-48.52 39.48-88 88-88s88 39.48 88 88c0 30.864-16.008 58.024-40.128 73.736a84.742 84.742 0 01-9.8 5.48c-.4.192-.792.392-1.192.576-23.168 10.536-50.592 10.536-73.76 0-.4-.184-.8-.384-1.192-.576a84.083 84.083 0 01-9.8-5.48C168.008 234.024 152 206.864 152 176zm269.832 194.584c-18.136-53.552-59.512-96.832-112.376-117.392C330.6 234.144 344 206.64 344 176c0-57.344-46.656-104-104-104s-104 46.656-104 104c0 30.64 13.4 58.144 34.552 77.192-52.864 20.568-94.24 63.84-112.376 117.392C31.672 333.792 16 288.704 16 240 16 116.488 116.488 16 240 16s224 100.488 224 224c0 48.704-15.672 93.792-42.168 130.584z" />
            </svg>
          </span>
          <span>Tài khoản</span>
        </div>
      ) : null}
      <div
        className={page === 6 ? "menu-item active" : "menu-item"}
        onClick={() => changePage("/quan-li-sinh-vat")}
      >
        <span className="icon">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 480 480"
            width={23}
            height={23}
          >
            <path d="M473.513 412h-6.558a67.7 67.7 0 0 1 9.755-25.079 48.912 48.912 0 0 1 2.951-4.066C470.915 387.688 461.7 397 458.282 412h-10.234a56.681 56.681 0 0 1 12.714-26.614 111.811 111.811 0 0 1 11.761-30.4 77.346 77.346 0 0 0-6.868 5.674C451.384 374.244 443.5 392 442.123 412h-14.466c-3.21-22-17.565-31.008-29.136-34.848a55.542 55.542 0 0 1 5.741 7.056c4.217 6.361 9.43 17.792 12.146 27.792H294v-63h-21v63H119V262.378a3.911 3.911 0 0 0-2.462-3.384 3.374 3.374 0 0 0-4.3 1.819 2.831 2.831 0 0 0-.241 1.069V309.9a5.079 5.079 0 0 1-3.805 4.833 5.01 5.01 0 0 1-5.629-2.355l-19-33.209a2.964 2.964 0 0 0-4.021-1.1 3.053 3.053 0 0 0-1.229 4.14l25.329 44.015A62.73 62.73 0 0 1 112 357.31V412h-10v-13.62a21.984 21.984 0 0 0-18.054-9.466 21.745 21.745 0 0 0-19.264 11.571A5.032 5.032 0 0 1 60 403.124c-.188-.008-.4-.214-.583-.214A16.042 16.042 0 0 0 45.244 412h-6.757c-8.184 0-15.109 6-17.28 13h469.586c-2.172-7-9.093-13-17.28-13zM326.839 229H239l-15.553 50h118.584zM283.434 86.147 260.773 159h44.797zM308.608 169h-50.945l-15.553 50h81.69z" />
            <path d="M442.228 375.455a84.629 84.629 0 0 1 4.921-8.128c.143-5.193-1.007-24.6-3.3-39.986-4.537 11.059-8.109 25.8-6.381 39.708a86.135 86.135 0 0 1 4.76 8.406zm-8.106 22.23a89.286 89.286 0 0 1 2.64-9.54A79.182 79.182 0 0 0 411.5 353.2a96.6 96.6 0 0 1 10.6 26.679 47.8 47.8 0 0 1 12.022 17.806zM345.069 289H220.337l-15.552 50h155.477zm-243.802 60.783a52.627 52.627 0 0 0-6.392-18.6l-25.236-44.049a13.056 13.056 0 0 1 4.894-17.734c.022-.012.024-.024.039-.033a13.079 13.079 0 0 1 17.739 4.854L102 291.049v-29.276c0-.06-.05-.119-.048-.178a13.263 13.263 0 0 1 1.029-4.6 13.514 13.514 0 0 1 17.542-7.2A13.908 13.908 0 0 1 129 262.457v87.194a72.874 72.874 0 0 0 58.907-71.776c0-40.314-33.011-73.112-73.325-73.112a73.116 73.116 0 0 0-13.315 145.02zm-.3-133.129a64.776 64.776 0 0 1 3.258-.572 5 5 0 0 1 1.472 9.891c-.91.135-1.835.3-2.748.483a5.068 5.068 0 0 1-1 .1 5 5 0 0 1-.986-9.9zM54.7 284.42a5 5 0 0 1-5-5 63.608 63.608 0 0 1 30.954-54.842 5 5 0 1 1 5.174 8.557A53.687 53.687 0 0 0 59.7 279.42a5 5 0 0 1-5 5z" />
          </svg>
        </span>
        <span>Quản lí sinh vật</span>
      </div>
      <div
        className={page === 7 ? "menu-item active" : "menu-item"}
        onClick={() => changePage("/mau-go")}
      >
        <span className="icon">
          <IconWood width="23px" height="23px" />
        </span>
        <span>Mẫu gỗ</span>
      </div>
      <div
        className={page === 8 ? "menu-item active" : "menu-item"}
        onClick={() => changePage("/dau-chan")}
      >
        <span className="icon">
          <IconFootPrint width="23px" height="23px" />
        </span>
        <span>Dấu chân</span>
      </div>
      {/* <div
        className={page === 9 ? "menu-item active" : "menu-item"}
        onClick={() => changePage("/feedbacks")}
      >
        <span className="icon">
          <IconFeedbacks width="23px" height="23px" />
        </span>
        <span>Feedbacks</span>
      </div> */}
      <div
        className={page === 10 ? "menu-item active" : "menu-item"}
        onClick={() => changePage("/questions")}
      >
        <span className="icon">
          <IconQuestion width="23px" height="23px" />
        </span>
        <span>Câu hỏi</span>
      </div>
      <div
        className={page === 11 ? "menu-item active" : "menu-item"}
        onClick={() => changePage("/events")}
      >
        <span className="icon">
          <IconEvent width="23px" height="23px" />
        </span>
        <span>Sự kiện</span>
      </div>
      <div
        className={page === 12 ? "menu-item active" : "menu-item"}
        onClick={() => changePage("/contributions")}
      >
        <span className="icon">
          <IconContributions width="23px" height="23px" />
        </span>
        <span>Đóng góp</span>
      </div>
    </div>
  );
};

export default connect(mapStateToProps, mapDispatchToProps)(SideBarAdmin);
